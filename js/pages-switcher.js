  ...
  var PageTransitions = (function ($, options) {
    ...
    function Animate($pageTrigger, gotoPage) {
      ...
      if (currentPageId != gotoPage) {
        ...
        var $nextPage = $("section[data-id=" + gotoPage + "]").addClass(
          "pt-page-current"
        );
        ...
        if (gotoPage === "resume") {
          jQuery(".skill-container").each((a, x) => {
            var val = jQuery(x).data("value");
            jQuery(x).find(".skill-percentage").css("width", `${val}%`);
            console.log("here", val);
          });
        } else {
          jQuery(".skill-container").each((a, x) => {
            var val = jQuery(x).data("value");
            jQuery(x).find(".skill-percentage").css("width", `0%`);
          });
        }
      }
...
/*
     FILE ARCHIVED ON 14:10:33 Jan 19, 2022 AND RETRIEVED FROM THE
...
playback timings (ms):
  ...
  esindex: 0.012
  cdx.remote: 91.844
  LoadShardBlock: 174.647 (3)
  PetaboxLoader3.datanode: 162.14 (4)
  PetaboxLoader3.resolve: 153.526 (2)
  load_resource: 192.107
*/
